...
import { HackerNewsNormalizer } from '@/lib/llm/normalizer';
import { calculateProblemScore } from '@/lib/utils/scoring';
import type { IngestionStats } from '@/types/api';

/**
 * POST /api/ingest/hackernews
 * 
 * Ingests new Ask HN posts and normalizes them into problems using LLM
 */
export async function POST(request: NextRequest) {
    const startTime = new Date();

    try {
        console.log('[Ingestion] Starting Hacker News ingestion...');

        // Get HN source from database
        const hnSource = await db.query.sources.findFirst({
            where: eq(sources.name, 'hackernews'),
        });

        if (!hnSource) {
            return NextResponse.json(
                { error: 'Hacker News source not found in database' },
                { status: 500 }
            );
        }
        ...
        const normalizer = new HackerNewsNormalizer();

        // Fetch new events (for now, fetch all recent ones; later we'll track 'since')
        ...
        for (const event of newEvents) {
            try {
                ...
                const existing = await db.query.rawEvents.findFirst({
                    where: and(
                        eq(rawEvents.sourceId, hnSource.id),
